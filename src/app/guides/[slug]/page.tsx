    ...
    <div className="min-h-screen py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/guides" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-card border border-border text-sm text-muted-foreground hover:text-foreground mb-6 transition-all">
          <ArrowLeft className="w-4 h-4" /> Alle Guides
        </Link>

        <div className="bg-card rounded-2xl border border-border p-8 md:p-12">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className={`text-xs px-3 py-1 rounded-full font-medium ${
              guide.level === 'Anfänger' ? 'bg-success/20 text-success' :
              guide.level === 'Mittel' ? 'bg-accent/20 text-accent' :
          ...
          {guide.videoId && (
            <div className="mt-10 p-6 rounded-2xl bg-accent/5 border border-accent/20">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold mb-1">Passendes YouTube-Video</h3>
                  <p className="text-sm text-muted-foreground mb-3">Sieh dir die Video-Version dieses Guides an:</p>
                  <a
                  ...
                  >
                    <ExternalLink className="w-4 h-4" /> Video ansehen
                  </a>
                </div>
              </div>
            </div>
